...
angular.module('angularLazyImg').factory('LazyImgMagic', [ '$window', 'lazyImgConfig', 'lazyImgHelpers',
...
angular.module('angularLazyImg').factory('lazyImgHelpers', [
  '$window', function($window){
    ...
    function isElementInView(elem, offset, winDimensions) {
      ...
      rect.bottom <= bottomline - offset && rect.bottom >= 0 - offset
      )
      );
    }
...
angular.module('angularLazyImg').directive('lazyImg', [
  'LazyImgMagic','$timeout', function(LazyImgMagic,$timeout){
    'use strict';
...
]).directive('lazyImgLay', ['lazyImgConfig', function(lazyImgConfig){
    'use strict';
    ...
    function link(scope, element, attributes) {
      ...
      lazyImgConfig.setOptions(options);
      //console.log(lazyImgConfig.setOptions)
    }
    return {
      link: link,
      restrict: 'A'
